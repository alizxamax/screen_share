...
class ScreenShareService {
    ...
    for (final track in _localStream!.getTracks()) {
      ...
      if (track.kind == 'video') {
        ...
        if (encodings.isNotEmpty) {
          ...
          // Resolution is already set by capture constraints above.
          // Avoid scaling again to prevent double downscaling quality loss.
  ...
  Future<_CropApplyResult> _applyCropIfSupported(StreamSettings settings) async {
    ...
    final croppedWidth = (settings.scaledWidth * (1 - left - right)).round().clamp(320, settings.scaledWidth);
    final croppedHeight = (settings.scaledHeight * (1 - top - bottom)).round().clamp(180, settings.scaledHeight);
    ...
    try {
      ...
      return _CropApplyResult('Crop applied on track: ${settings.crop}', true);
    } catch (_) {
      // Fallback for platforms that do not support edge crop fields.
      // We still force output size/aspect so receiver gets a framed stream.
      try {
